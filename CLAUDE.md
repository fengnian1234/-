...
**用户提到某个文件夹名称时，必须优先使用 Glob/Read/Bash 查看该文件夹的内容**，再决定后续操作。禁止跳过本地文件直接搜索网络资源或凭空猜测文件夹内有什么。用户通常会事先准备好相关资源放在文件夹中，直接去用它即可。
...
|-----|-----|---------|------|------|
| 归墅 | `guishu` | `#7B5D3B` | 茶褐 | 陈年普洱，温润沉稳 |
| 山纪 | `shanji` | `#2C4335` | 墨绿 | 深林地绿，禅意茶室 |
