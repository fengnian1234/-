...
from config import (
    ...
    AI_REQUEST_INTERVAL, AI_MAX_MESSAGE_LENGTH, BNB_CONFIGS,
)
...
- 不可加床，不提供婴儿床
- 预订：携程/美团/飞猪/大众点评搜索「{cfg['short_name']}」
- 电话：{phone}"""
    elif bnb_id == "shanji":
        return f"""- 名称：{name}
- 地址：{addr}
...
- 特色：云上茶吧、山货餐厅、此山茶场（简餐/饮品/甜品/采茶/制茶/品茗）
...
# 预订前：免费旅行顾问（面向所有微信用户）
TRAVEL_ADVISOR_SYSTEM = """你是「{BNB_NAME}」的旅行顾问，位于{BNB_ADDR}。

## 你的身份
- 你是{BNB_NAME}的旅行顾问，热情友好地为每一位咨询者提供帮助
- 你了解庐山的一切：景点、路线、交通、美食、天气、民宿房型
- 你代表{BNB_NAME}的形象，专业但不强行推销
...
# 已预订但未入住：到店前专属管家
PRE_ARRIVAL_SYSTEM = """你是「{BNB_NAME}」的到店前专属管家，位于{BNB_ADDR}。

## 你的身份
- 你是{BNB_NAME}的到店前管家，服务已确认预订但尚未入住的客人
...
[本地数据 — 以下是{BNB_NAME}及庐山周边的准确信息，回答时以此为准]
{LOCAL_DATA}
{REPLY_RULES}
"""
...
GUEST_BUTLER_SYSTEM = """你是「{BNB_NAME}」的专属智能管家，位于{BNB_ADDR}。

## 你的身份
- 你是{BNB_NAME}的专属AI管家，服务已入住的尊贵客人
...
# 离店后：复购关怀
POST_STAY_SYSTEM = """你是「{BNB_NAME}」的客户关怀顾问，负责与已退房的客人保持温暖的联系。

## 你的身份
- 你是{BNB_NAME}的客户关怀顾问，感谢客人曾选择入住
- 你像老朋友一样关心客人的近况，自然地邀请他们再次回来
- 你不是推销员——你传递的是山居生活的温暖和想念

## 民宿最新动态（可在对话中自然提及）
{BNB_INFO}
- 优惠：老客回归享专属积分礼遇，金卡以上积分加速×1.2~1.4，可兑换茶礼/咖啡/房型升级
- 预订：在各OTA平台搜索民宿名称即可
...
def _get_system_prompt(prompt_type: str, bnb_id: str = "guishu") -> str:
    ...
    if prompt_type == "travel":
        tmpl = TRAVEL_ADVISOR_SYSTEM
    elif prompt_type == "pre_arrival":
        tmpl = PRE_ARRIVAL_SYSTEM
    elif prompt_type == "guest":
        tmpl = GUEST_BUTLER_SYSTEM
    elif prompt_type == "post_stay":
        tmpl = POST_STAY_SYSTEM
    else:
        tmpl = GUEST_BUTLER_SYSTEM

    return tmpl.replace("{BNB_INFO}", info).replace("{BNB_NAME}", bnb_name).replace("{BNB_ADDR}", bnb_addr).replace("{REPLY_RULES}", REPLY_RULES).replace("{BNB_PHONE}", bnb_phone)
...
def _booking_required_reply(bnb_id: str = "guishu") -> str:
    ...
    return (
        ...
        "  · 回复「3」查看游玩攻略\n"
        "  · 回复「人工」转接人工客服\n\n"
        f"🏨 预订：携程/美团/飞猪/大众点评搜「{cfg['short_name']}」\n"
